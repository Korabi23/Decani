...
router.patch("/job-applications/:id", requireAdmin, async (req, res, next) => {
  try {
    const updated = await JobApplication.findByIdAndUpdate(req.params.id, { status: req.body.status }, { new: true });
...
router.get("/business-suggestions", requireAdmin, async (req, res, next) => {
  try { res.json(await BusinessSuggestion.find({}).sort({ createdAt: -1 })); }
  catch (e) { next(e); }
});

router.patch("/business-suggestions/:id", requireAdmin, async (req, res, next) => {
  try {
    const updated = await BusinessSuggestion.findByIdAndUpdate(req.params.id, { status: req.body.status }, { new: true });
    if (!updated) return res.status(404).json({ message: "Not found" });
    res.json(updated);
  } catch (e) { next(e); }
});
